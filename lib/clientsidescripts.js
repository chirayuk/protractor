...
clientSideScripts.waitForAngular = function(selector, callback) {
  // ckck_log("waitForAngular: ENTER");
  var el = document.querySelector(selector);
  function waitLoop() {
    if (angular) {
      // ckck_log("waitForAngular: angular is present");
      angular.element(el).injector().get('$testability').
          // ckck notifyWhenNoOutstandingRequests(callback);
          notifyWhenNoOutstandingRequests(function() {
            // ckck_log("waitForAngular: calling callback");
            callback();
          });
    } else {
  ...
  try {
    ...
    ckck_log("waitForAngular: **** EXCEPTION: (continuing) ****\n"+e);
    callback(e);
  }
};
...
clientSideScripts.findBindings = function(binding, using) {
  ...
  nodes = ensureElements(nodes);
  ckck_log("findBindings: binding: "+binding+": Found: "+nodes);
  return nodes;
};
 ...
 */
 clientSideScripts.findAllRepeaterRows = function(repeater, using) {
  ...
  // ckck_log("findAllRepeaterRows: repeater: " + repeater + ", using: " + using);
      ...
      for (var i = 0; i < repeatElems.length; ++i) {
        ...
        if (containerMode) {
          if (elem.getAttribute(attr).indexOf(repeater) != -1) {
            // ckck_log("findAllRepeaterRows: rows.push(elem=" + elem + ")");
            rows.push(elem);
          }
          continue;
        }
...
clientSideScripts.findRepeaterElement = function(repeater, index, binding, using) {
  var matches = [];
  using = using || document;
  ckck_log("findRepeaterElement: repeater: " + repeater + ", index: " + index + ", binding: " + binding + ", using: " + using);
  ...
  for (var p = 0; p < prefixes.length; ++p) {
    ...
    for (var i = 0; i < repeatElems.length; ++i) {
      if (repeatElems[i].getAttribute(attr).indexOf(repeater) != -1) {
        rows.push(repeatElems[i]);
        ckck_log("findRepeaterElement: rows.push(" + repeatElems[i] + ")");
      }
    }
  }
  // multiRows is an array of arrays, where each inner array contains
  ...
  for (var p = 0; p < prefixes.length; ++p) {
    ...
    for (var i = 0; i < repeatElems.length; ++i) {
      if (repeatElems[i].getAttribute(attr).indexOf(repeater) != -1) {
        ...
        multiRows.push(row);
        ckck_log("findRepeaterElement: multiRows.push(" + row + ")");
      }
    }
  }
  var row = rows[index];
...
clientSideScripts.findByModel = function(model, using) {
  using = using || document;
  // ckck_log("findByModel: ENTER: model: %O, using: %O"+ model+ using);
  var prefixes = ['ng-', 'ng_', 'data-ng-', 'x-ng-', 'ng\\:'];
  for (var p = 0; p < prefixes.length; ++p) {
    var selector = '[' + prefixes[p] + 'model="' + model + '"]';
    var elements = using.querySelectorAll(selector);
    // ckck_log("findByModel: elements.length: " + elements.length + ", elements: " + elements);
    if (elements.length) {
      return elements;
    }
  }
...
clientSideScripts.enableAnimations = function(element, value) {
  return angular.element(element).enableAnimations(value);
};
