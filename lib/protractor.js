var ckck_log = exports.ckck_log = require('ckck').new_logger("Protractor");
var ckck_log_js = require('ckck').get_js_to_inject_into_browser("ClientSideScript");
...
var clientSideScriptsJs = {};
// Without the following, any errors in *_spec.dart code results in just
// a "Unknown error" stack trace with a very unhelpful dart2js traceback
// that does not list the dart code traceback.  This twiddling gives us
// both the correct exception message as well as the dart stack trace.
for (var script in clientSideScripts) {
  clientSideScriptsJs[script] = util.format(
    'try { return (%s).apply(this, arguments); }\n' +
    'catch(e) { throw new Error(e); }',
     clientSideScripts[script].toString());
}
...
var buildElementHelper = function(ptor, opt_usingChain) {
  ...
  var element = function(locator) {
    ...
    var webElementFns = WEB_ELEMENT_FUNCTIONS.concat(
        ['findElements', 'isElementPresent', 'evaluate', '$$', 'enableAnimations']);
...
Protractor.prototype.waitForAngular = function() {
  ...
  return this.driver.executeAsyncScript(
    clientSideScriptsJs.waitForAngular, this.rootEl).then(function(browserErr) {
      if (browserErr) {
...
Protractor.prototype.wrapWebElement = function(element) {
  ...
  element.evaluate = function(expression) {
    thisPtor.waitForAngular();
    return element.getDriver().executeScript(clientSideScriptsJs.evaluate,
        element, expression);
  };

  element.enableAnimations = function(value) {
    ckck_log("enableAnimations", value);
    thisPtor.waitForAngular();
    return element.getDriver().executeScript(clientSideScriptsJs.enableAnimations,
        element, value);
  };
...
Protractor.prototype.get = function(destination, opt_timeout) {
  ...
  this.driver.executeScript(ckck_log_js);
  ...
  self.driver.executeAsyncScript(clientSideScriptsJs.testForAngular, timeout).
...
Protractor.prototype.getLocationAbsUrl = function() {
  this.waitForAngular();
  return this.driver.executeScript(clientSideScriptsJs.getLocationAbsUrl, this.rootEl);
};
...
Protractor.prototype.debugger = function() {
  ...
  for (var script in clientSideScripts) {
    clientSideScriptsList.push(
      script + ': ' + clientSideScripts[script]);
  }
...
Protractor.prototype.ckck_dbg = function() {
  debugger;
}
